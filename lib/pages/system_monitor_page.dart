...
class _SystemMonitorPageState extends State<SystemMonitorPage> {
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      if (_battery < 0 || _batteryTemp == null) await _loadBatteryFallback();
      if (_thermal.isEmpty) await _readThermalFallback();
    } finally {
      _refreshing = false;
    }
  }
  ...
  void _applyNative(Map<String, dynamic> data) {
    ...
    final online = <String>[];
    final rawOnline = data['online_cpu_list'] ?? data['online_cpus'];
    ...
    if (rawThermal is List) {
      for (final item in rawThermal) {
        if (item is Map) {
          final name = item['type']?.toString() ?? item['name']?.toString() ?? 'thermal';
          final value = double.tryParse(item['temperature']?.toString() ?? '');
          if (value != null && value >= -40 && value <= 150) thermal[name] = value;
        }
      }
    } else if (rawThermal is Map) {
  ...
  Future<void> _readThermalFallback() async {
    try {
      final directory = Directory('/sys/class/thermal');
      if (!await directory.exists()) return;
      final values = <String, double>{};
      for (final zone in directory.listSync(followLinks: false)) {
        if (zone is! Directory || !zone.path.contains('thermal_zone')) continue;
        try {
          final raw = int.tryParse(await File('${zone.path}/temp').readAsString());
          if (raw == null) continue;
          final value = raw / 1000.0;
          if (value >= -40 && value <= 150) values[zone.path.split('/').last] = value;
        } catch (_) {}
      }
      if (values.isNotEmpty && mounted) setState(() => _thermal = values);
    } catch (_) {}
  }
  ...
  Widget _metric(String title, String value, IconData icon) => _card(Padding(
  ...
  Widget build(BuildContext context) {
    ...
    final used = _ramTotal <= 0 ? 0.0 : (_ramTotal - _ramAvailable).clamp(0, _ramTotal).toDouble();
    final ramPercent = _ramTotal <= 0 ? 0.0 : used / _ramTotal * 100;
    final model = _text(_device['model']);

    return Scaffold(
      appBar: AppBar(title: const Text('Monitor'), centerTitle: true, actions: [IconButton(onPressed: _refresh, icon: const Icon(Icons.refresh))]),
      body: RefreshIndicator(
        onRefresh: () async { await _loadDevice(); await _loadBatteryFallback(); await _readThermalFallback(); await _refresh(); },
        child: ListView(
          physics: const AlwaysScrollableScrollPhysics(),
          padding: const EdgeInsets.fromLTRB(16, 8, 16, 24),
          children: [
            _card(Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(model, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w800)),
            ...
            _metric('RAM', '${used.toStringAsFixed(1)} / ${_ramTotal.toStringAsFixed(1)} GB (${ramPercent.toStringAsFixed(0)}%)', Icons.memory_outlined),
            const SizedBox(height: 8),
            ...
            const SizedBox(height: 7),
            _card(Padding(padding: const EdgeInsets.all(16), child: _frequencies.isEmpty
                ? const Text('Frekans bilgisi erişilemiyor.')
                : Column(children: _frequencies.take(16).toList().asMap().entries.map((e) => Padding(padding: const EdgeInsets.symmetric(vertical: 2), child: Row(children: [Expanded(child: Text('CPU ${e.key}')), Text(_freq(e.value), style: const TextStyle(fontWeight: FontWeight.w700))]))).toList()))),
            const SizedBox(height: 14),
            Text('Termal sensörler', style: TextStyle(fontWeight: FontWeight.w800, color: Theme.of(context).colorScheme.onSurfaceVariant)),
            const SizedBox(height: 7),
            _card(Padding(padding: const EdgeInsets.all(16), child: _thermal.isEmpty
                ? const Text('Termal sensör bilgisi bu cihaz/ROM tarafından sunulmuyor.')
            ...
            const SizedBox(height: 7),
            _card(Padding(padding: const EdgeInsets.all(16), child: Column(children: [
              _info('Model', model),
              _info('Marka', _text(_device['brand'])),
              _info('SoC', _text(_device['soc_model'])),
              _info('ABI', _text(_device['supported_abis'])),
              _info('CPU çekirdek', _text(_device['cpu_count'])),
              _info('Ekran', '${_text(_device['screen_width'])} × ${_text(_device['screen_height'])}'),
              _info('Yenileme', '${_text(_device['refresh_rate'])} Hz'),
              _info('Güvenlik yaması', _text(_device['security_patch'])),
              _info('Batarya kaynağı', _batterySource),
            ]))),
          ],
        ),
      ),
